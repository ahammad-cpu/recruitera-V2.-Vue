<!--
  Career site — OPPORTUNITIES. Public job listing with filters (location, job
  type, category, work type), search, and list/grid views. Roles from useJobs()
  (published only); themed by useCareerSite().
-->
      ...
      <div class="mt-8 grid gap-8 lg:grid-cols-[260px_1fr] items-start">
        ...
        <div>
          ...
          <div class="mt-5" :class="view === 'grid' ? 'grid gap-4 sm:grid-cols-2' : 'flex flex-col gap-4'">
            <NuxtLink v-for="job in results" :key="job.id" :to="`/career-site/jobs/${job.id}`"
              class="block rounded-[16px] border border-[#eceef1] bg-white p-5 transition hover:shadow-[0_16px_40px_rgba(15,23,42,0.07)]">
