<!--
  Career site — JOB DETAIL + APPLY. Overview tab (description, responsibilities,
  requirements) and Application tab (personal info, resume, screening, submit).
  Job from useJobs() by route id; themed by useCareerSite().
-->
...
const { jobs } = useJobs()
const job = computed(() => jobs.value.find(j => j.id === String(route.params.id)))
...
<template>
  <CareerShell>
    <div v-if="job" class="mx-auto max-w-[900px] px-6 py-12">
      <NuxtLink to="/career-site/opportunities" class="inline-flex items-center gap-1.5 text-[14px] font-semibold transition hover:opacity-70" :style="{ color: 'var(--cc-primary)' }"><ArrowLeft class="w-4 h-4" stroke-width="2" /> Back</NuxtLink>
    ...
    <div v-else class="mx-auto max-w-[900px] px-6 py-24 text-center">
      ...
      <NuxtLink to="/career-site/opportunities" class="mt-3 inline-block text-[14px] font-semibold" :style="{ color: 'var(--cc-primary)' }">← Back to opportunities</NuxtLink>
    </div>
  </CareerShell>
</template>
