<!--
  Career site — HOME. Modern candidate landing: cover hero, overlapping
  "Featured Jobs" card, values, culture video (YouTube embed), editorial
  testimonials, employee CTA. Every section + color is driven by
  useCareerSite() (Settings → Career Site); roles come from useJobs().
-->
...
<template>
  <CareerShell hero-overlap>
    <!-- Hero: cover image or gradient (full-bleed, behind the header) -->
    <section class="relative overflow-hidden">
      ...
      <div class="relative mx-auto max-w-[1160px] px-6 pt-32 pb-40 md:pt-40 md:pb-48 text-white">
        ...
        <div class="mt-9 flex flex-wrap gap-3">
          ...
          <NuxtLink to="/career-site/opportunities" class="inline-flex items-center h-12 px-6 rounded-[13px] border-[1.5px] border-white/40 text-[15px] font-semibold text-white hover:bg-white/10 transition">Browse all roles</NuxtLink>
        </div>
      </div>
    </section>
        ...
        <div class="rounded-[22px] bg-white border border-[#eceef1] shadow-[0_28px_70px_rgba(15,23,42,0.12)] p-7 md:p-10">
          ...
          <div class="mt-7 grid gap-4 md:grid-cols-2 lg:grid-cols-3">
            <NuxtLink v-for="job in featured" :key="job.id" :to="`/career-site/jobs/${job.id}`"
              class="group flex flex-col rounded-[16px] border border-[#eceef1] bg-white p-5 transition hover:border-[color:var(--cc-primary)] hover:shadow-[0_16px_40px_rgba(15,23,42,0.08)]">
